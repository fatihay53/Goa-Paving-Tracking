...
preJobSafetyRouter.post('/save', function (request, response) {
    var sql = `INSERT INTO pre_job_safety_form (date,foremanId,location,signatureForeman,estimate_template_id,general_options,environment_options,hazardous_options,others,task_list,possible_hazard_list,hazard_control_list,created_date) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`;
    ...
    connection.query(sql, [date,foremanId, location, signatureForeman, estimateTemplateId, generalOptions, environmentOptions, hazardousOptions, others,taskList,possibleHazardList,hazardControlList,createdDate], function (error, rows) {
...
preJobSafetyRouter.get('/findAll', function (request, response) {
    var sql = `select p.*,e.project_name as project_name,em.name as foremanName,em.surname as foremanSurname from pre_job_safety_form p left join estimate_template e on p.estimate_template_id = e.id join users u on p.foremanId=u.id join employee em on u.id=em.user_id order by created_date desc`;
